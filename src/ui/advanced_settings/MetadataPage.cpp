...
MetadataPage::MetadataPage(ConfigManager *configManager, QWidget *parent)
    ...
    m_embedThumbnailCheck->setToolTip(tr("Store the downloaded thumbnail or artwork inside the media file when the output format supports it. Audio artwork keeps its original composition while obvious borders around centered square artwork are removed automatically."));
...
void MetadataPage::loadSettings() {
    ...
    QSignalBlocker b5(m_generateFolderJpgCheck);
    QSignalBlocker b6(m_forcePlaylistAsAlbumSwitch);
