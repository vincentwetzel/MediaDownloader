...
- `YtDlpArgsBuilder.h/.cpp` - "Helper"; Constructs command-line arguments for `yt-dlp` based on settings from `ConfigManager`. **Now relies solely on `ConfigManager` for the `temporary_downloads_directory` and `output_template`.** **Download sections preserve the user's requested output container (for example MP4) and only add `--force-keyframes-at-cuts` for video/livestream downloads; the app no longer forces an intermediate MKV remux for clipped sections.** **If a section label is provided, it appends a filename-safe suffix such as `[section 15-00_to_end]` before `%(ext)s` so clipped files identify which part of the source they represent.** **It now also normalizes legacy codec labels like `H.264`/`H.265`, translates video/audio codec preferences into yt-dlp selectors that match real stream aliases such as `avc1`, `hev1`, and `mp4a`, honors direct runtime `format` overrides from `FormatSelectionDialog`, merges separate runtime video/audio format IDs for video downloads, and respects the `restrict_filenames` Advanced Setting when building yt-dlp arguments.**
