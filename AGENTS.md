...
- **Queue State / App Restarts**: `src/core/DownloadQueueState.h/.cpp` (serializes active, paused, and stopped queue items including their `tempFilePath`, `originalDownloadedFilePath`, and cleanup-related per-item options to `downloads_backup.json` so partial downloads and temp-file cleanup survive application restarts).
