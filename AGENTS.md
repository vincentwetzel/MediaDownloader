...
- `ActiveDownloadsTab.h/.cpp` - Monitoring; renders active/completed downloads, progress bars, etc.
...
- **Download Item Widget**: `src/ui/DownloadItemWidget.h/.cpp` (now correctly updates the title label when a title is received in `updateProgress`).
