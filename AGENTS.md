...
- `advanced_settings/BinariesPage.h/.cpp` - Advanced Settings page for external dependency management. **Shows per-binary status for discovered/configured executables, keeps manual "Browse" overrides, and offers package-manager/manual-download install actions for `yt-dlp`, `ffmpeg`, `ffprobe`, `gallery-dl`, `aria2c`, and `deno`.** **yt-dlp install suggestions now prefer nightly-capable commands where available (for example Scoop `yt-dlp-nightly` or Homebrew `--HEAD`) and clearly label when a package manager only provides stable builds.**
...
- `DownloadSectionsDialog.h/.cpp` - Runtime section picker; builds one or more chapter/time-range clip definitions for `yt-dlp --download-sections` and also produces a filename-safe section label for clipped outputs.
