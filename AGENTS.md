...
- **Process Execution (`yt-dlp`/`ffmpeg`)**: `src/YtDlpJsonExtractor.h/.cpp` and `src/FfmpegPostProcessor.h/.cpp` (asynchronous metadata and muxing). **Shared process-tree shutdown logic lives in `src/core/ProcessUtils.h/.cpp`, and app exit is coordinated from `src/core/DownloadManager.h/.cpp` plus `src/ui/MainWindow.h/.cpp`.**
