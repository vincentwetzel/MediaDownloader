...
- `advanced_settings/BinariesPage.h/.cpp` - Advanced Settings page for external dependency management. **Shows per-binary status for discovered/configured executables, keeps manual "Browse" overrides (use "Clear Path" to reset to auto-detection), and offers package-manager/manual-download "Install" actions for `yt-dlp`, `ffmpeg`, `ffprobe`, `gallery-dl`, `aria2c`, and `deno`.** **yt-dlp install suggestions now prefer nightly-capable commands where available (for example Scoop `yt-dlp-nightly` or Homebrew `--HEAD`) and clearly label when a package manager only provides stable builds.**
