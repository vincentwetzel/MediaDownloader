...
`build-debug` tree. If compiler metadata is incomplete or the cache was
created with a non-Ninja generator, use `cmake --fresh --preset debug` or
`tools/configure_debug.ps1`; do not create a parallel debug build tree.
