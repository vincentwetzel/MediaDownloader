...
| `playlist_logic` | String | `Ask` | Default playlist handling mode. Options: `Ask`, `Download All (no prompt)`, `Download Single (ignore playlist)`. A caller-supplied request-level `playlist_logic` value takes precedence over this default; invalid values become `Ask`. In `Ask` mode, detected multi-item playlists can be queued entirely, narrowed through the partial range/checkbox selector, reduced to the first item, or cancelled. Non-interactive requests always use `Download All (no prompt)`. |
...
| `embed_thumbnail` | Boolean | `true` | Embed thumbnail images into downloaded files as cover art. Audio sidecars are normalized before embedding; containers supported by the app-side rewrite are then remuxed by the app, while formats such as Opus, Ogg/Vorbis, ADTS AAC, and WAV use yt-dlp's native embedding path. |
...
If a required binary (`yt-dlp`, `ffmpeg`, `ffprobe`, `deno`) is not found, LzyDownloader opens the **Set Up Required Tools** checklist during startup, enqueue, or format checks. The checklist keeps recommended install actions inline, distinguishes fresh installs from existing-binary upgrades, and offers **Update All** for supported automatic actions. The same install and browse actions are also available from Advanced Settings → External Tools.
