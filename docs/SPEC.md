...
- Browser-companion messages use protocol `1` and bounded, length-prefixed
  ...
  `UNSUPPORTED_PROTOCOL`. Browser-companion enqueue requests may carry a
  validated, request-scoped cookie-file path and client identity. The path is
  passed only to the worker as `--cookies`, is excluded from queue backups and
  API status payloads, and is removed on terminal/removal cleanup; status and
  cancellation are scoped to the client identity. Cookie bundles are capped
  at 500 entries/900 KiB, fields are bounded and control-character-free, each
